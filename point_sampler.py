...
class PointSampler: 
    ...
    def conc_at_point(self, downwind_distance, crosswind_distance, wind_speed, stability_class, emission_rate):
        ...
        if np.any(positive_distance_index): 
            ...
            denominator = self.calculate_denominator(positive_wind_speed, standard_deviations[1]) 
            numerator = self.calculate_numerator(emission_rate, standard_deviations, positive_crosswind_distance)
    ...
    def calculate_error_functions(self, crosswind_distance_km, standard_deviation_y):
        right_y = (crosswind_distance_km + self.source_width / 2) / (np.sqrt(2) * standard_deviation_y)
